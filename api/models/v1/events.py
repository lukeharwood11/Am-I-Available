...
class EventDataDateTime(BaseModel):
    ...
    date_time: str | None = Field(
        None, alias="dateTime", description="RFC3339 datetime string"
    )
    date: str | None = Field(
        None, description="Date string for all-day events (YYYY-MM-DD)"
    )
    time_zone: str | None = Field(
        None, alias="timeZone", description="Time zone (e.g., 'America/New_York')"
    )
...
class EventDataAttendee(BaseModel):
    ...
    display_name: str | None = Field(
        None, alias="displayName", description="Attendee display name"
    )
    ...
    additional_guests: int | None = Field(
        None, alias="additionalGuests", description="Number of additional guests"
    )
...
class EventDataReminders(BaseModel):
    ...
    use_default: bool = Field(
        True, alias="useDefault", description="Use calendar default reminders"
    )
    overrides: list[EventDataReminder] = Field(
...
class EventDataAttachment(BaseModel):
    ...
    mime_type: str | None = Field(
        None, alias="mimeType", description="MIME type of the file"
    )
...
class EventDataConferenceCreateRequest(BaseModel):
    ...
    conference_solution_key: dict[str, Any] = Field(
        alias="conferenceSolutionKey", description="Conference solution details"
    )
...
class EventDataConferenceData(BaseModel):
    ...
    create_request: EventDataConferenceCreateRequest | None = Field(
        None, alias="createRequest", description="Request to create conference"
    )
    entry_points: list[dict[str, Any]] = Field(
        default_factory=list, alias="entryPoints", description="Conference entry points"
    )
    conference_solution: dict[str, Any] | None = Field(
        None, alias="conferenceSolution", description="Conference solution info"
    )
    conference_id: str | None = Field(
        None, alias="conferenceId", description="Conference ID"
    )
...
class EventData(BaseModel):
    ...
    attendees: list[EventDataAttendee] = Field(
        default_factory=list, description="Event attendees"
    )
    recurrence: list[str] = Field(
        default_factory=list, description="Recurrence rules (RRULE format)"
    )
    reminders: EventDataReminders | None = Field(None, description="Reminder settings")
    attachments: list[EventDataAttachment] = Field(
        default_factory=list, description="File attachments"
    )
    conference_data: EventDataConferenceData | None = Field(
        None, alias="conferenceData", description="Conference/meeting data"
    )
    visibility: str | None = Field(
        None, description="Event visibility: default, public, private, confidential"
    )
    transparency: str | None = Field(
        None, description="Event transparency: opaque, transparent"
    )
    event_type: str | None = Field(
        None,
        alias="eventType",
        description="Event type: default, outOfOffice, focusTime",
    )
    color_id: str | None = Field(
        None, alias="colorId", description="Color ID for the event"
    )
    ...
    guests_can_modify: bool | None = Field(
        None, alias="guestsCanModify", description="Whether guests can modify the event"
    )
