...
from .databridge.event_request_approvals_databridge import EventRequestApprovalsDatabridge
...
def get_relationships_databridge(supabase: Client = Depends(get_supabase_admin_client)) -> RelationshipsDatabridge:
    """Dependency to get relationships databridge instance"""
    return RelationshipsDatabridge(supabase=supabase)


def get_relationship_metadata_databridge(supabase: Client = Depends(get_supabase_admin_client)) -> RelationshipMetadataDatabridge:
    """Dependency to get relationship metadata databridge instance"""
    return RelationshipMetadataDatabridge(supabase=supabase)


def get_relationship_requests_databridge(supabase: Client = Depends(get_supabase_admin_client)) -> RelationshipRequestsDatabridge:
    """Dependency to get relationship requests databridge instance"""
    return RelationshipRequestsDatabridge(supabase=supabase)


def get_event_requests_databridge(supabase: Client = Depends(get_supabase_admin_client)) -> EventRequestsDatabridge:
    """Dependency to get event requests databridge instance"""
    return EventRequestsDatabridge(supabase=supabase)


def get_event_request_approvals_databridge(supabase: Client = Depends(get_supabase_admin_client)) -> EventRequestApprovalsDatabridge:
    """Dependency to get event request approvals databridge instance"""
    return EventRequestApprovalsDatabridge(supabase=supabase)


def get_user_token_databridge(supabase: Client = Depends(get_supabase_admin_client)) -> UserTokenDatabridge:
    """Dependency to get user token databridge instance"""
    return UserTokenDatabridge(supabase=supabase)


# Service Dependencies
def get_relationships_service(databridge: RelationshipsDatabridge = Depends(get_relationships_databridge)) -> RelationshipsService:
    """Dependency to get relationships service instance"""
    return RelationshipsService(databridge=databridge)


def get_relationship_metadata_service(databridge: RelationshipMetadataDatabridge = Depends(get_relationship_metadata_databridge)) -> RelationshipMetadataService:
    """Dependency to get relationship metadata service instance"""
    return RelationshipMetadataService(databridge=databridge)


def get_relationship_requests_service(databridge: RelationshipRequestsDatabridge = Depends(get_relationship_requests_databridge)) -> RelationshipRequestsService:
    """Dependency to get relationship requests service instance"""
    return RelationshipRequestsService(databridge=databridge)


def get_event_requests_service(databridge: EventRequestsDatabridge = Depends(get_event_requests_databridge)) -> EventRequestsService:
    """Dependency to get event requests service instance"""
    return EventRequestsService(databridge=databridge)


def get_event_request_approvals_service(databridge: EventRequestApprovalsDatabridge = Depends(get_event_request_approvals_databridge)) -> EventRequestApprovalsService:
    """Dependency to get event request approvals service instance"""
    return EventRequestApprovalsService(databridge=databridge)


def get_google_events_service(user_token_databridge: UserTokenDatabridge = Depends(get_user_token_databridge)) -> GoogleEventsService:
