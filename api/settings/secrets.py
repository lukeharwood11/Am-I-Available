...
class SecretsManager:
    ...
    def get_secret(self, *, name: str, default: str | type(...) = ...) -> str:
        ...
        """
        print(f"Getting secret '{name}' from SECRETS_MANAGER_SECRET_NAME: '{self.secrets_name}' with environment '{self.environment}'")
        if self.environment == "prod":
            return self._secrets_dict.get(name, default) if default is not ... else self._secrets_dict[name]
        else:
