...
class GoogleEventsService:
    ...
    async def _get_user_tokens(self, *, user_id: str) -> DBUserTokenResponse:
        ...
        if not token_data:
            raise ValueError("No Google tokens found for user. Please connect your Google account.")
        return token_data
    ...
    async def get_current_week_events(self, *, user_id: str) -> list[CalendarEvent]:
        ...
        try:
            ...
            logger.error(f"Error getting current week events for user {user_id}: {str(e)}")
            raise
